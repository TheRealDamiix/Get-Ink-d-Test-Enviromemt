...
const PortfolioManager = () => {
  ...
  const handleAddImage = async () => {
    ...
    try {
      const imageBase64 = await convertFileToBase64(newImageFile);
      
      const { data: uploadData, error: uploadError } = await supabase.functions.invoke('upload-to-cloudinary', {
        body: { imageBase64, userId: user.id, caption: newImageCaption },
      });

      if (uploadError || !uploadData || !uploadData.secure_url) {
        throw new Error(uploadError?.message || uploadData?.error || 'Failed to upload image to Cloudinary.');
      }
