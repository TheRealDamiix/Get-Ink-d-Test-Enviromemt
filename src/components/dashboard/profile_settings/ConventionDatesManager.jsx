...
import { Loader2, PlusCircle, Trash2, Edit, CalendarDays } from 'lucide-react';
...
const ConventionDatesManager = ({ user }) => {
  ...
  const [currentConvention, setCurrentConvention] = useState({ id: null, event_name: '', location: '', start_date: '', end_date: '' });
  ...
  const handleOpenConventionDialog = (convention = null) => {
    ...
    if (convention) {
      ...
      setCurrentConvention({ id: null, event_name: '', location: '', start_date: '', end_date: '' });
      ...
      {isConventionLoading && conventionDates.length === 0 ? (
        ...
        <div className="space-y-4">
          {conventionDates.map(conv => (
            <div key={conv.id} className="flex items-center justify-between p-3 border border-border/50 rounded-md">
              <div>
