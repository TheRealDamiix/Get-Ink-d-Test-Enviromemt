...
const UserPostsManager: React.FC<UserPostsManagerProps> = ({ user, onPostCreatedOrUpdated }) => {
  ...
  const handleOpenPostDialog = (post: PostRecord | null = null) => {
    if (!user?.id) {
  ...
  const uploadImageToCloudinaryFormData = async (file: File, folderName: string, fileNamePrefix: string = 'image'): Promise<{ url: string; publicId: string } | null> => {
    ...
    try {
      ...
      if (uploadError || !uploadData || !(uploadData as Record<string, unknown>).secure_url) {
        let errorMessage = `Failed to upload ${folderName} image.`;
        if (uploadError?.message) errorMessage = uploadError.message;
        else if ((uploadData as Record<string, unknown>)?.error) errorMessage = (uploadData as Record<string, unknown>).error as string;
      ...
      toast({ title: "Upload Error", description: `Failed to upload ${folderName} image. ${(error as Error).message}`, variant: "destructive" });
      return null;
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSavePost = async () => {
    ...
    let imageUrl: string | null = currentPost.existing_image_url;
    let imagePublicId: string | null = currentPost.existing_public_id;
        ...
        <DialogContent className="glass-effect sm:max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <Input id="post-title" value={currentPost.title} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCurrentPost({ ...currentPost, title: e.target.value })} placeholder="Post title..." />
            </div>
            <div>
              <Label htmlFor="post-content">Content</Label>
              <Textarea id="post-content" value={currentPost.content} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setCurrentPost({ ...currentPost, content: e.target.value })} placeholder="What's on your mind?" className="min-h-[120px]" />
            </div>
            <div>
