...
import type { ConventionDate, AuthUser } from '@/types';
...
interface ConventionDatesManagerProps {
  user: AuthUser | null;
}

const defaultForm: ConventionFormState = {
  id: null,
  event_name: '',
  location: '',
  start_date: '',
...
const ConventionDatesManager = ({ user }: ConventionDatesManagerProps) => {
  ...
  const [currentConvention, setCurrentConvention] = useState<ConventionFormState>(defaultForm);
  ...
  const fetchConventionDates = useCallback(async () => {
    ...
    if (error) {
      ...
      setConventionDates((data as ConventionDate[]) || []);
    }
    setIsConventionLoading(false);
  }, [user, toast]);
  ...
  const handleOpenConventionDialog = (convention: ConventionDate | null = null) => {
    if (!user?.id) {
      toast({ title: "User not loaded", description: "Cannot manage convention dates without user data.", variant: "destructive" });
      return;
    }
    if (convention) {
      const convRow = convention as ConventionDate & { event_name?: string };
      setCurrentConvention({
        id: convention.id,
        event_name: convRow.event_name || convention.convention_name,
        location: convention.location,
        start_date: convention.start_date,
        end_date: convention.end_date || '',
        start_time: (convention as unknown as Record<string, string>).start_time || '10:00',
        end_time: (convention as unknown as Record<string, string>).end_time || '20:00',
        accepting_bookings: (convention as unknown as Record<string, boolean>).accepting_bookings ?? true,
        is_fully_booked: (convention as unknown as Record<string, boolean>).is_fully_booked || false,
      });
      setIsEditingConvention(true);
    } else {
      setCurrentConvention(defaultForm);
  ...
  const handleDeleteConvention = async (conventionId: string) => {
    setIsConventionLoading(true);
  ...
  // Cast conv to access extended fields not in base ConventionDate type
  type ConvRow = ConventionDate & { event_name?: string; start_time?: string; end_time?: string; accepting_bookings?: boolean; is_fully_booked?: boolean };
    ...
    <div className="mt-8 p-6 glass-effect rounded-xl">
      ...
      {isConventionLoading && conventionDates.length === 0 ? (
        ...
        <div className="space-y-4">
          {(conventionDates as ConvRow[]).map(conv => (
            <div key={conv.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 border border-border/50 rounded-md gap-2">
              <div>
                <p className="font-medium">{conv.event_name || conv.convention_name}</p>
