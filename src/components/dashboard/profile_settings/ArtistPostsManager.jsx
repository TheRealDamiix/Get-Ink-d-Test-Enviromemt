...
import { Loader2, PlusCircle, Trash2, Edit, Image as ImageIcon, Newspaper } from 'lucide-react';
...
const UserPostsManager = ({ user, onPostCreatedOrUpdated }) => {
  ...
  const handleSavePost = async () => {
    if (!user?.id || (!currentPost.content.trim() && !currentPost.title.trim())) { // Allow post if title or content exists
    ...
    if (currentPost.image_file) {
      ...
      if (uploaded) {
        ...
        if (isEditingPost && currentPost.existing_public_id && currentPost.existing_public_id !== imagePublicId) {
           await supabase.functions.invoke('delete-from-cloudinary', { body: { publicId: currentPost.existing_public_id } });
        }
      } else {
        setIsLoading(false);
    ...
    const payload = {
      user_id: user.id,
      title: currentPost.title.trim() || null, // Added
  ...
  const handleDeletePost = async (post) => {
    ...
    if (dbError) {
      ...
      if (post.image_public_id) {
        try {
          await supabase.functions.invoke('delete-from-cloudinary', { body: { publicId: post.image_public_id } });
        ...
        <DialogContent className="glass-effect sm:max-w-lg">
          ...
          <div className="space-y-4 py-4">
            ...
            <div>
              ...
              <Input id="post-image" type="file" accept="image/*" onChange={handleImageFileChange} className="mt-1" />
