...
interface CurrentDeal {
  ...
  existing_image_public_id: string | null;
  valid_until: string;
}
...
interface ArtistDealsManagerProps {
  user: AuthUser;
  onDealCreatedOrUpdated?: () => void;
}

const ArtistDealsManager: React.FC<ArtistDealsManagerProps> = ({ user, onDealCreatedOrUpdated }) => {
  const [deals, setDeals] = useState<DealRecord[]>([]);
  const [showDealDialog, setShowDealDialog] = useState<boolean>(false);
  const [currentDeal, setCurrentDeal] = useState<CurrentDeal>({
    id: null,
    title: '',
    deal_description: '',
    price: '',
  ...
  const [isEditingDeal, setIsEditingDeal] = useState<boolean>(false);
  ...
  const handleOpenDealDialog = (deal: DealRecord | null = null) => {
    if (!user?.id) {
  ...
  const uploadImageToCloudinary = async (file: File): Promise<{ url: string; publicId: string } | null> => {
    ...
    try {
      ...
      if (uploadError || (uploadData as Record<string, unknown>).error || !(uploadData as Record<string, unknown>).secure_url) {
        const errorMessage = uploadError?.message || (uploadData as Record<string, unknown>)?.error as string || 'Failed to upload deal image.';
  ...
  const deleteImageFromCloudinary = async (publicId: string) => {
    ...
    try {
      ...
      console.warn(`Could not delete old image ${publicId} from Cloudinary:`, (error as Error).message);
    }
  };
  ...
  const handleSaveDeal = async () => {
    ...
    let imageUrl: string | null = currentDeal.existing_image_url;
    let imagePublicId: string | null = currentDeal.existing_image_public_id;
  ...
  const handleDeleteDeal = async (deal: DealRecord) => {
    setIsLoading(true);
        ...
        >
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <Input id="deal-title" value={currentDeal.title} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCurrentDeal({ ...currentDeal, title: e.target.value })} placeholder="e.g., 20% Off Flash Tattoos" />
            </div>
            <div>
              <Label htmlFor="deal-description">Description (Optional)</Label>
              <Textarea id="deal-description" value={currentDeal.deal_description} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setCurrentDeal({ ...currentDeal, deal_description: e.target.value })} placeholder="Details about the deal..." className="min-h-[100px]" />
            </div>
            <div>
              <Label htmlFor="deal-price">Price (Optional)</Label>
              <div className="relative">
                <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <Input id="deal-price" type="number" step="0.01" value={currentDeal.price} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCurrentDeal({ ...currentDeal, price: e.target.value })} placeholder="e.g., 50.00" className="pl-8" />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                <Input id="deal-valid-until" type="date" value={currentDeal.valid_until} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCurrentDeal({ ...currentDeal, valid_until: e.target.value })} className="pl-10" min={new Date().toISOString().split('T')[0]} />
              </div>
            </div>
          </div>
