...
const AvailabilityManager = () => {
  ...
  useEffect(() => {
    const initialAvailability: AvailabilityState = daysOfWeek.reduce<AvailabilityState>((acc, day) => {
      acc[day] = { active: false, start: '09:00', end: '17:00' };
      return acc;
    }, {});

    const userAvailability = (user?.profile?.general_availability ?? {}) as Record<string, Partial<DaySchedule>>;
  ...
  const handleSaveAvailability = async () => {
    setIsSaving(true);
    try {
      const { error } = await updateUser({ general_availability: availability as Record<string, unknown> });
      if (error) throw error;
      toast({ title: "Availability Saved!", description: "Your general booking hours have been updated." });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Unknown error';
      toast({ title: "Error", description: `Could not save availability: ${message}`, variant: "destructive" });
    } finally {
      setIsSaving(false);
    }
  };
