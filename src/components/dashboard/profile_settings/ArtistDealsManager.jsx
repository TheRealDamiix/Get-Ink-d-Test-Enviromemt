...
const ArtistDealsManager = ({ user, onDealCreatedOrUpdated }) => {
  ...
  const [currentDeal, setCurrentDeal] = useState({ 
    id: null, 
    title: '', 
    deal_description: '', // Changed from description
    price: '', // Added price
    image_file: null, 
  ...
  const handleOpenDealDialog = (deal = null) => {
    ...
    if (deal) {
      setCurrentDeal({
        id: deal.id,
        title: deal.title || '',
        deal_description: deal.deal_description || '', // Changed
        price: deal.price !== null && deal.price !== undefined ? String(deal.price) : '', // Added
        image_file: null,
        existing_image_url: deal.image_url,
        existing_public_id: deal.image_public_id,
        valid_until: deal.valid_until ? deal.valid_until.split('T')[0] : '' // Format for date input
      });
      setIsEditingDeal(true);
    } else {
      setCurrentDeal({ id: null, title: '', deal_description: '', price: '', image_file: null, existing_image_url: null, existing_public_id: null, valid_until: '' });
  ...
  const uploadImageToCloudinaryFormData = async (file, folderName, fileNamePrefix = 'image') => {
    ...
    setIsLoading(true); // Ensure loading state is managed during upload
    try {
      ...
      const { data: uploadData, error: uploadError } = await supabase.functions.invoke('upload-to-cloudinary', {
        body: formDataForUpload, // Send FormData directly
      });
      ...
      if (uploadError || !uploadData || !uploadData.secure_url) {
        ...
        else if (uploadData?.error) errorMessage = uploadData.error; // Check for error in response data
  ...
  const handleSaveDeal = async () => {
    ...
    let imagePublicId = currentDeal.existing_public_id;

    if (currentDeal.image_file) {
      const uploaded = await uploadImageToCloudinaryFormData(currentDeal.image_file, 'artist_deals', 'deal');
      if (uploaded) {
        imageUrl = uploaded.url;
        imagePublicId = uploaded.publicId;
        if (isEditingDeal && currentDeal.existing_public_id && currentDeal.existing_public_id !== imagePublicId) {
           await supabase.functions.invoke('delete-from-cloudinary', { body: { publicId: currentDeal.existing_public_id } });
        }
      } else {
        setIsLoading(false);
        return; // Stop if image upload failed
      }
    } else if (!currentDeal.existing_image_url && isEditingDeal && currentDeal.existing_public_id) {
      // If image was removed (no new file, no existing URL, but had public_id)
      try {
        await supabase.functions.invoke('delete-from-cloudinary', { body: { publicId: currentDeal.existing_public_id } });
        imageUrl = null;
        imagePublicId = null;
    ...
    const payload = {
      ...
      deal_description: currentDeal.deal_description.trim() || null, // Changed
      price: !isNaN(dealPrice) ? dealPrice : null, // Added, ensure it's a number or null
      image_url: imageUrl,
      image_public_id: imagePublicId,
      valid_until: currentDeal.valid_until ? new Date(currentDeal.valid_until).toISOString() : null, // Ensure ISO format for TIMESTAMPTZ
    };

    let error;
    if (isEditingDeal && currentDeal.id) {
      // payload.updated_at = new Date().toISOString(); // DB trigger handles this
  ...
  const handleDeleteDeal = async (deal) => {
    ...
    if (dbError) {
      ...
      if (deal.image_public_id) {
        try {
          await supabase.functions.invoke('delete-from-cloudinary', { body: { publicId: deal.image_public_id } });
        } catch (cloudinaryError) {
          toast({ title: "Warning", description: "Deal deleted from database, but failed to remove image from Cloudinary.", variant: "default" });
        }
      }
              ...
              {currentDeal.existing_image_url && (
                <div className="mt-2 relative w-40 h-40">
                  ...
                  <Button 
                    ...
                    onClick={() => setCurrentDeal(prev => ({...prev, image_file: null, existing_image_url: null, existing_public_id: null }))}
                  >
