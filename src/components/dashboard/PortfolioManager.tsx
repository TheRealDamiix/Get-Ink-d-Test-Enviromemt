...
const PortfolioManager: React.FC<PortfolioManagerProps> = () => {
  ...
  const handleAddImage = async () => {
    ...
    try {
      ...
      if (uploadError) {
        ...
        } else if (typeof uploadError === 'object' && (uploadError as Record<string, unknown>).error) {
          errorMessage = (uploadError as Record<string, unknown>).error as string;
        }
        throw new Error(errorMessage);
      }

      if (!uploadData || !(uploadData as Record<string, unknown>).secure_url) {
        console.error("Upload data missing URL:", uploadData);
        throw new Error("Image upload failed to return URL.");
      }

      const { secure_url, public_id } = uploadData as { secure_url: string; public_id: string };
      ...
      setPortfolioImages(prev => [dbData as PortfolioImageItem, ...prev]);
      setNewImageFile(null);
      setNewImageCaption('');
  ...
  const handleRemoveImage = async (imageToRemove: PortfolioImageItem) => {
    ...
    try {
      const { error: cloudinaryError } = await supabase.functions.invoke('delete-from-cloudinary', {
        body: { public_ids: [imageToRemove.cloudinary_public_id] }, // Ensure public_ids is an array
      });
          ...
          <DialogContent className="max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea id="image-caption" value={newImageCaption} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setNewImageCaption(e.target.value)} placeholder="Describe this tattoo..." rows={3} />
              </div>
            </div>
            <DialogFooter>
              <DialogClose asChild>
