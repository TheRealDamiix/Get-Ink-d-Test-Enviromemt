...
const ArtistPostsDisplay = ({ artistId, artistUsername, artistName, artistProfilePhotoUrl }) => {
  ...
  const fetchPosts = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('posts') // Changed from 'artist_posts'
      .select('*') // Artist info is now passed as props, or fetched separately if needed for each post
      .eq('user_id', artistId) // Changed from 'artist_id'
      .order('created_at', { ascending: false })
      .limit(5); // Limit to latest 5 posts for profile display
          ...
          >
            ...
            {post.image_url && (
              <div className="mb-3 rounded-md overflow-hidden max-h-80">
                <img-replace src={post.image_url} alt="Post image" className="w-full h-full object-cover" />
              </div>
            )}
