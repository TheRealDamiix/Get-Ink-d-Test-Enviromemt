...
const ImageDialog = ({ selectedImage, artist, onOpenChange }) => {
  ...
  const [newTagPosition, setNewTagPosition] = useState(null); // { x_percent, y_percent }
  ...
  const handleUserSelectedForTag = async (taggedUser) => {
    ...
    try {
      ...
      setNewTagPosition(null);
      setIsTaggingMode(false); // Optionally turn off tagging mode after one tag
    }
  };
  ...
  return (
    <>
      <Dialog open={!!selectedImage} onOpenChange={(isOpen) => { onOpenChange(isOpen); if (!isOpen) setIsTaggingMode(false); }}>
        <DialogContent className="max-w-3xl glass-effect p-0">
          <div className="grid grid-cols-1 md:grid-cols-3">
            <div className={`md:col-span-2 aspect-[4/3] md:aspect-auto relative ${isTaggingMode ? 'cursor-crosshair' : ''}`} onClick={handleImageClickForTagging}>
              <img-replace
                ref={imageRef}
              ...
              <div className="mt-auto space-y-3">
                {canManageTags && (
                  <Button variant={isTaggingMode ? "destructive" : "outline"} className="w-full" onClick={() => setIsTaggingMode(!isTaggingMode)}>
                    <Tag className="w-4 h-4 mr-2" /> {isTaggingMode ? "Cancel Tagging" : "Tag People"}
                  </Button>
                )}
