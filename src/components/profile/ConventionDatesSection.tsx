...
interface ConventionDatesSectionProps {
  artistId: string;
  artistProfile: Partial<Profile> | null;
  dates: ConventionDateRow[];
  loading: boolean;
}

const formatTourDate = (startDateStr: string, endDateStr?: string | null): string => {
...
const ConventionDatesSection = ({ artistId, artistProfile, dates, loading }: ConventionDatesSectionProps) => {
  const { toast } = useToast();
  const { user } = useAuth();
  const navigate = useNavigate();
  const [showBookingDialog, setShowBookingDialog] = useState(false);
  const [selectedConventionForBooking, setSelectedConventionForBooking] = useState<ConventionDateRow | null>(null);

  const handleRequestBookingForConvention = (convention: ConventionDateRow) => {
    if (!user) {
  ...
  const artistName = ((artistProfile as Record<string, unknown>)?.name as string | undefined) ?? artistProfile?.username ?? undefined;
    ...
    <>
          ...
          {dates.map((event) => (
            <div key={event.id} className="flex items-center justify-between p-2 border-b border-border/20 last:border-b-0">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-foreground">{event.event_name || event.convention_name}</h3>
      ...
      <Dialog open={showBookingDialog} onOpenChange={setShowBookingDialog}>
        <DialogContent className="glass-effect sm:max-w-lg max-h-[90vh] overflow-y-auto custom-scrollbar">
          <DialogHeader>
            <DialogTitle>Request Booking for {selectedConventionForBooking?.event_name || selectedConventionForBooking?.convention_name}</DialogTitle>
          </DialogHeader>
          <BookingRequestForm
            artistId={artistId}
