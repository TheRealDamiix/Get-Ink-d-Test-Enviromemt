...
interface ReviewWithArtist {
  id: string;
  stars: number;
  comment: string | null;
  created_at: string;
  artist?: ProfileSummary;
}
...
const UserReviewsDialog = ({ userProfile, open, onOpenChange }: UserReviewsDialogProps) => {
  const [reviews, setReviews] = useState<ReviewWithArtist[]>([]);
    ...
    const fetchUserReviews = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          setReviews((data as ReviewWithArtist[]) || []);
        }
      } catch (e) {
                    ...
                    {review.artist && (
                      ...
                      >
                        <Avatar className="w-12 h-12 border-2 border-primary/30">
                          <AvatarImage src={review.artist.profile_photo_url ?? undefined} alt={review.artist.name ?? undefined} />
