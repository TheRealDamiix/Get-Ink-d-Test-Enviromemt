...
interface ReviewsSectionProps {
  ...
  onReviewAdded: (review: { stars: number; comment: string }) => Promise<void>;
}

const REVIEWS_TO_SHOW_INITIALLY = 2;

const ReviewsSection = ({ reviews, artistId, artistProfile, onReviewAdded }: ReviewsSectionProps) => {
  const { user } = useAuth();
  ...
  const handleOpenUserReviews = (reviewer: ReviewerSummary | undefined) => {
    if (!reviewer) return;
                ...
                <div className="flex items-start gap-4">
                  <Link to={review.reviewer?.username ? `/user/${review.reviewer.username}` : '#'}>
                    <Avatar className="w-10 h-10">
                      <AvatarImage src={review.reviewer?.profile_photo_url ?? undefined} alt={review.reviewer?.name ?? undefined} />
