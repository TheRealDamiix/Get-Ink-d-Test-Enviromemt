...
const FollowersDialog = ({ artistId, artistName, open, onOpenChange }: FollowersDialogProps) => {
    ...
    const fetchFollowers = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          setFollowers(
            (data as unknown as Array<{ follower: FollowerProfile | null }>)
              .map(f => f.follower)
              .filter((f): f is FollowerProfile => f !== null)
          );
        }
      } catch (e) {
                ...
                <li key={follower.id}>
                  ...
                  >
                    <Avatar className="w-12 h-12 border-2 border-transparent group-hover:border-primary/50 transition-all">
                      <AvatarImage src={follower.profile_photo_url ?? undefined} alt={follower.name ?? undefined} />
