...
interface ArtistBookingListProps {}

const ArtistBookingList: React.FC<ArtistBookingListProps> = () => {
  const { user } = useAuth();
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      toast({ title: `Error ${actionType}ing booking`, description: (error as Error).message, variant: "destructive" });
    } finally {
  ...
  const openActionDialog = (booking: Booking, type: string) => {
    ...
    <div className="space-y-6">
      {bookings.map(booking => (
        <div key={booking.id} className="p-6 glass-effect rounded-xl border border-border/50">
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h4 className="text-lg font-semibold">
                  Request from: {(booking.client as Record<string, unknown>)?.name as string || (booking.client as Record<string, unknown>)?.username as string || 'Unknown Client'}
                </h4>
              </div>
              {!!(booking.client as Record<string, unknown>)?.email && <p className="text-sm text-muted-foreground mb-1">Email: {(booking.client as Record<string, unknown>).email as string}</p>}
              {!!(booking as unknown as Record<string, unknown>).client_phone_number && <p className="text-sm text-muted-foreground mb-1"><Phone className="w-4 h-4 inline mr-1.5 text-foreground" /> Phone: {(booking as unknown as Record<string, unknown>).client_phone_number as string}</p>}
              <p className="text-sm text-muted-foreground mb-1">
                <Clock className="w-4 h-4 inline mr-1.5 text-foreground" />
                Requested: {formatDate((booking as unknown as Record<string, unknown>).requested_datetime as string)}
              </p>
              <p className="text-sm text-muted-foreground">
                Status: <span className={`font-medium ${
                  booking.status === 'pending' ? 'text-yellow-400' :
                  booking.status === 'confirmed' ? 'text-green-400' :
                  (booking.status as string).includes('declined') || (booking.status as string).includes('cancelled') ? 'text-red-500' : 'text-foreground'
                }`}>{(booking.status as string).replace(/_/g, ' ')}</span>
              </p>
          ...
          {!!(booking as unknown as Record<string, unknown>).convention_date_id && <p className="text-xs mt-2 text-blue-400">This booking is related to a convention/event.</p>}
          {!!(booking as unknown as Record<string, unknown>).reference_image_url && (
            <div className="mt-3 pt-3 border-t border-border/30">
              <p className="text-sm font-medium mb-1">Reference Image:</p>
              <button onClick={() => setImageToPreview((booking as unknown as Record<string, unknown>).reference_image_url as string)} className="cursor-pointer">
                <img src={(booking as unknown as Record<string, unknown>).reference_image_url as string} alt="Booking reference" className="rounded-md max-w-[150px] max-h-[150px] object-cover border border-border" />
              </button>
            </div>
          )}
        </div>
      ...
      <Dialog open={!!selectedBooking} onOpenChange={() => setSelectedBooking(null)}>
        <DialogContent className="glass-effect max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to {actionType} this booking request from {(selectedBooking?.client as Record<string, unknown>)?.name as string || 'this client'} for {formatDate((selectedBooking as unknown as Record<string, unknown>)?.requested_datetime as string)}?
            </DialogDescription>
          </DialogHeader>
          {actionType === 'confirm' && (
            <div className="space-y-2 py-4">
                ...
                <Input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setBookingDuration(parseInt(e.target.value, 10) || 0)}
                    placeholder="e.g., 60"
                />
            </div>
          )}
