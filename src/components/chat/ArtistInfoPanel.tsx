...
const ArtistInfoPanel: React.FC<ArtistInfoPanelProps> = ({ artist, bookings }) => {
  if (!artist) return null;

  const upcomingBookings = Array.isArray(bookings)
    ? bookings.filter(b => new Date((b as unknown as Record<string, unknown>).requested_datetime as string) > new Date())
    : [];
            ...
            <div className="space-y-3">
                {upcomingBookings.map(booking => (
                    <div key={booking.id} className="text-xs p-2.5 rounded-md bg-background/50">
                        ...
                        <p className="text-muted-foreground flex items-center mt-1"><Clock className="w-3 h-3 mr-1.5" />{formatDate((booking as unknown as Record<string, unknown>).requested_datetime as string)}</p>
