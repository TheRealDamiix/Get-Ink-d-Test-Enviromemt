...
const UserInfoPanel: React.FC<UserInfoPanelProps> = ({ client, artist, bookings }) => {
  ...
  const now = new Date();
  const upcomingBookings = bookings.filter(b => new Date((b as unknown as Record<string, unknown>).requested_datetime as string) >= now).sort((a, b) => new Date((a as unknown as Record<string, unknown>).requested_datetime as string).getTime() - new Date((b as unknown as Record<string, unknown>).requested_datetime as string).getTime());
  const pastBookings = bookings.filter(b => new Date((b as unknown as Record<string, unknown>).requested_datetime as string) < now).sort((a, b) => new Date((b as unknown as Record<string, unknown>).requested_datetime as string).getTime() - new Date((a as unknown as Record<string, unknown>).requested_datetime as string).getTime());
    ...
    <div className="w-full h-full bg-card/40 p-4 flex flex-col custom-scrollbar overflow-y-auto">
      ...
      <div className="text-center mb-4 flex-shrink-0">
        <Avatar className="w-20 h-20 mx-auto mb-3 border-4 border-secondary shadow-lg">
          <AvatarImage src={client.profile_photo_url ?? undefined} alt={client.name ?? undefined} />
                        ...
                        {upcomingBookings.map(b => (
                            <div key={b.id} className="text-xs p-2 rounded-md bg-background/50 mb-2">
                                <div className="flex justify-between items-center">
                                    <p className="font-semibold">{formatDate((b as unknown as Record<string, unknown>).requested_datetime as string)}</p>
