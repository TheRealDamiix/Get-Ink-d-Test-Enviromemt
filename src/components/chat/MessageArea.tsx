...
const MessageBubble: React.FC<MessageBubbleProps> = ({ msg, currentUserId, onImageClick }) => {
...
interface MessageAreaProps {
  ...
  otherUser: Pick<Profile, 'id' | 'name' | 'username' | 'profile_photo_url' | 'is_artist'> | null;
}

const MessageArea: React.FC<MessageAreaProps> = ({ conversationId, otherUser }) => {
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            ...
            try {
                ...
                toast({ title: "Error Loading Messages", description: (error as Error).message, variant: "destructive" });
            } finally {
                setIsLoading(false);
            }
        };
    ...
    useEffect(() => {
        ...
        const subscription = channel.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `conversation_id=eq.${conversationId}` },
            (payload) => {
                if ((payload.new as Record<string, unknown>).sender_id === otherUser?.id) {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        if (attachedImageFile) {
            ...
            if (error || (data as Record<string, unknown>).error) {
                toast({ title: "Upload Error", description: `Failed to upload image: ${error?.message || (data as Record<string, unknown>).error}`, variant: "destructive" });
                setIsSending(false);
                return;
            }
            imageUrl = (data as Record<string, unknown>).secure_url as string;
            imagePublicId = (data as Record<string, unknown>).public_id as string;
        }
        ...
        <div className="flex flex-col h-full flex-1">
             <header className="p-3 border-b border-border/50 flex items-center gap-3 flex-shrink-0 bg-card/20">
                <Link to={profileLink}>
                    <Avatar className="h-9 w-9">
                        <AvatarImage src={otherUser.profile_photo_url ?? undefined} alt={otherUser.name ?? otherUser.username} />
            ...
            <footer className="p-4 border-t border-border/50 flex-shrink-0 bg-card/20">
                ...
                <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                    ...
                    <Input type="text" placeholder="Type a message..." value={newMessage} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewMessage(e.target.value)} disabled={isSending} className="flex-1 bg-background" />
