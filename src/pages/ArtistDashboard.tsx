...
import { Eye, RefreshCw, Loader2, Settings, Image, BookOpenCheck } from 'lucide-react'; // Removed Newspaper
...
// UserPostsManager is now part of ProfileSettings for artists
// ArtistDealsManager is now part of ProfileSettings for artists
...
const ArtistDashboard = () => {
  ...
  const [stats, setStats] = useState<DashboardStats>({ followers_count: 0, portfolio_count: 0, reviews_count: 0, total_rating_sum: 0 });
  const [isStatsLoading, setIsStatsLoading] = useState<boolean>(true);
  const [activeTab, setActiveTab] = useState<string>(searchParams.get('tab') || "profile");

  const isArtist: boolean = user?.is_artist ?? (user?.profile?.is_artist ?? false);
  const userId: string | undefined = user?.id;
  const userUsername: string = user?.username ?? (user?.profile?.username ?? '');
    ...
    const fetchStats = async () => {
      if (userId && isArtist) {
        ...
        try {
          ...
          const totalRatingSum = reviewsData ? reviewsData.reduce((sum: number, review: { stars: number }) => sum + review.stars, 0) : 0;
  ...
  const artistProfileForStats = {
    ...
    ...stats
  } as import('@/types').Profile & { reviews_count?: number; total_rating_sum?: number; followers_count?: number; portfolio_count?: number };
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
          <TabsList className="grid w-full grid-cols-3 mb-6 glass-effect">
            <TabsTrigger value="profile" className="flex items-center gap-2">
              <Settings className="w-4 h-4" /> Profile & Content
            </TabsTrigger>
...
export const meta: PageMeta = {
  title: 'Artist Dashboard | InkSnap',
};
