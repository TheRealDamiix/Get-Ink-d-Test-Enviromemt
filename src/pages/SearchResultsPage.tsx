...
interface ArtistSearchResult extends Profile {
...
const SearchResultsPage = () => {
  ...
  const [locationTerm, setLocationTerm] = useState<string>(searchParams.get('location') || '');
  const [results, setResults] = useState<ArtistSearchResult[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const { toast } = useToast();

  const performSearch = useCallback(async (currentSearchTerm: string, currentSearchLocation: string) => {
    setIsLoading(true);
    setError(null);
    setResults([]);
    ...
    if (currentSearchLocation && currentSearchLocation.trim() !== '') {
      ...
      try {
        ...
        if (geoError) {
          console.warn("Geocoding error for search:", geoError.message || (geoData as { error?: string })?.error);
        } else if (geoData && (geoData as { latitude?: number; longitude?: number }).latitude && (geoData as { latitude?: number; longitude?: number }).longitude) {
          searchLat = (geoData as { latitude: number }).latitude;
          searchLon = (geoData as { longitude: number }).longitude;
    ...
    try {
      ...
      setResults((data as ArtistSearchResult[]) || []);
    } catch (err: unknown) {
            ...
            <div className="flex flex-col md:flex-row gap-0 p-1.5 rounded-2xl border border-primary/15 shadow-xl shadow-black/30"
              ...
              <div className="flex items-center flex-1 px-2 py-1">
                <LocationAutocomplete
                  ...
                  onSelect={(suggestion: { display: string }) => {
...
export const meta: PageMeta = {
  title: 'Search Artists | InkSnap',
  description: 'Search for tattoo artists by style, location, and more.',
};
