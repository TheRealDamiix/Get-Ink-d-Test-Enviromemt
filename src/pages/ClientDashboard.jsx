...
import { Heart, MapPin, Star, Clock, Settings } from 'lucide-react';
    ...
    if(user) {
      ...
      const fetchData = async () => {
        const { data: artists, error } = await supabase
          .from('follows')
          .select('artist:following_id(*)')
          .eq('follower_id', user.id);
        
        if (error) console.error('Error fetching followed artists:', error);
        else setFollowedArtists(artists.map(f => f.artist));
