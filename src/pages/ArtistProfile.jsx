...
const ArtistProfile = () => {
  ...
  const handleBookingRequest = () => {
    ...
    // setShowBookingDialog(true); // Enable this when form is fully functional
    toast({
      title: "🚧 Booking Coming Soon!",
      description: "The booking request form will be available shortly. Stay tuned!",
      duration: 5000,
    });
  };
    ...
    <div className="min-h-screen py-8 px-4">
      <div className="container mx-auto max-w-4xl">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <ArtistHeader artist={artistHeaderData} reviews={reviews} isFollowing={isFollowing} handleFollow={handleFollow} />
        </motion.div>

        {!canEdit && artist.booking_status && (
          <motion.div 
          ...
          >
            <Button onClick={handleBookingRequest} className="ink-gradient px-8 py-6 text-lg">
              <CalendarPlus className="w-5 h-5 mr-2" /> Request Booking
            </Button>
          </motion.div>
        )}

        <ConventionDatesSection artistId={artist.id} />
