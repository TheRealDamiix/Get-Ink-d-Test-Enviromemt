...
import type { PageMeta, Profile, Review, PortfolioImage, ConventionDate } from '@/types';
import type { PortfolioImageRow } from '@/components/profile/PortfolioGrid';
import type { ArtistInfoCardProfile as ArtistInfoCardProfileType } from '@/components/profile/ArtistInfoCard';
...
const ArtistProfile = () => {
  ...
  const [conventionDates, setConventionDates] = useState<ConventionDate[]>([]);
  const [nextConvention, setNextConvention] = useState<ConventionDate | null>(null);
  const [isFollowing, setIsFollowing] = useState<boolean>(false);
  const [followerCount, setFollowerCount] = useState<number>(0);
  const [selectedImage, setSelectedImage] = useState<PortfolioImageRow | null>(null);
  ...
  const fetchArtistData = useCallback(async (signal: AbortSignal) => {
    ...
    try {
      ...
      const allConventions: ConventionDate[] = (conventionsData as ConventionDate[]) || [];
      ...
      console.error('Error fetching artist profile data:', err.message);
      toastRef.current({ title: 'An unexpected error occurred', description: err.message, variant: 'destructive' });
      setArtist(null);
    } finally {
  ...
  const handleReviewAdded = async (reviewData: Partial<Review>) => {
    ...
    try {
      ...
      if (rpcError) {
        if (rpcError.message.includes('violates unique constraint') || rpcError.message.includes('violates check constraint')) {
          ...
          if ((existingConv as { id?: string })?.id) {
            navigate(`/chat?conversationId=${(existingConv as { id: string }).id}`);
            return;
          }
        }
        throw rpcError;
      ...
      if (conversationData && (conversationData as { conversation_id?: string }).conversation_id) {
        navigate(`/chat?conversationId=${(conversationData as { conversation_id: string }).conversation_id}`);
      } else {
        toast({ title: 'Error', description: 'Could not start or find conversation.', variant: 'destructive' });
      }
    } catch (error: unknown) {
      const err = error as { message?: string };
      toast({ title: 'Error Starting Chat', description: `An unexpected error occurred: ${err.message}`, variant: 'destructive' });
    }
  };
    ...
    <div className="min-h-screen py-8 px-4">
      <div className="container mx-auto max-w-4xl">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <ArtistInfoCard artist={artist as unknown as ArtistInfoCardProfileType} nextConvention={nextConvention} />
        </motion.div>
      ...
      <ImageDialog selectedImage={selectedImage} artist={artist} onOpenChange={(isOpen: boolean) => !isOpen && setSelectedImage(null)} />
