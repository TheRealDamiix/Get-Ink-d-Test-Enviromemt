...
interface FeedPost {
  ...
  poster_name?: string;
  poster_profile_photo_url?: string | null;
  poster_is_artist?: boolean;
  poster?: {
    id?: string;
    username?: string;
    name?: string;
...
const PostCard = ({ post }: PostCardProps) => {
  ...
  const timeSince = (dateString: string | null | undefined): string | null => {
      ...
      <div className="p-5">
        ...
        <Link to={profileLink} className="flex items-center gap-3 mb-4 group/author">
          <Avatar className="w-10 h-10 border-2 border-primary/30 group-hover/author:border-primary/60 transition-colors shrink-0">
            <AvatarImage src={post.poster_profile_photo_url ?? undefined} alt={post.poster_name} />
...
const NewsFeedPage = () => {
  ...
  const [showCreatePostDialog, setShowCreatePostDialog] = useState<boolean>(false);
  ...
  const fetchPosts = useCallback(async (currentPage: number, initialLoad = false) => {
    ...
    try {
      ...
      const formattedData: FeedPost[] = ((data as FeedPost[]) || []).map(p => ({
        ...p,
          ...
          >
            <div className="flex items-center gap-3">
              <Avatar className="border-2 border-primary/30 shrink-0">
                <AvatarImage src={user.profile_photo_url || user.profile?.profile_photo_url || undefined} alt={user.name || user.username} />
              ...
              <Button className="ink-gradient shrink-0 shadow-md shadow-primary/20" size="sm" onClick={(e: React.MouseEvent) => { e.stopPropagation(); setShowCreatePostDialog(true); }}>
...
export const meta: PageMeta = {
  title: 'Feed | InkSnap',
  description: 'Latest posts from tattoo artists you follow.',
};
