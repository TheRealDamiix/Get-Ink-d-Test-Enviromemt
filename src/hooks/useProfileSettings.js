...
export const useProfileSettings = () => {
  ...
  const buildFormDataFromUser = useCallback((currentUser) => {
    if (!currentUser) return null;
    const profileSource = currentUser.profile || currentUser;
    return {
      ...
      bookedUntil: profileSource.booked_until
        ? new Date(profileSource.booked_until).toISOString().split('T')[0]
        : '',
  ...
  // This prevents the form being reset while a user is mid-edit.
  ...
  const handleProfilePhotoChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.size > 5 * 1024 * 1024) {
        toastRef.current({ title: 'Image too large', description: 'Image must be less than 5MB.', variant: 'destructive' });
        return;
      }
  ...
  const uploadImageToCloudinary = async (file, folderName) => {
    ...
    try {
      ...
      if (uploadError || uploadData?.error) {
        throw new Error(uploadError?.message || uploadData?.error || `Failed to upload ${folderName} image.`);
      }
      return { url: uploadData.secure_url, publicId: uploadData.public_id };
    } catch (error) {
      toastRef.current({ title: 'Upload Error', description: error.message, variant: 'destructive' });
      return null;
    } finally {
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      ...
      toastRef.current({ title: 'Profile Updated', description: 'Your profile has been successfully updated.' });
    } catch (error) {
      toastRef.current({ title: 'Update Error', description: error.message, variant: 'destructive' });
    } finally {
      setIsSaving(false);
      setProfilePhotoFile(null);
      setLocationThumbnailFile(null);
  ...
  return {
    ...
    user,
    displayIsArtist: user?.profile?.is_artist ?? user?.is_artist ?? false,
  };
};
