...
export const AuthProvider = ({ children }) => {
  ...
  const fetchFullUserProfile = async (sessionUser) => {
    if (!sessionUser) {
    ...
    const rawMeta = sessionUser.raw_user_meta_data || {};
    ...
    if (fullUser.username === undefined) fullUser.username = rawMeta.username;
    if (fullUser.name === undefined) fullUser.name = rawMeta.name;
  ...
  const fetchUserProfile = async (userId) => {
    ...
    if (profileError && profileError.code !== 'PGRST116') {
      ...
      setUser(currentUser => {
        ...
        } else if (currentUser.raw_user_meta_data && typeof currentUser.raw_user_meta_data.is_artist === 'boolean') {
            determinedIsArtist = currentUser.raw_user_meta_data.is_artist;
        }
  ...
  const updateUserDatabase = async (updatedData) => {
    ...
    const fieldsToRemove = ['id', 'email', 'created_at', 'aud', 'role', 'app_metadata', 'user_metadata', 'identities', 'factors', 'is_anonymous', 'phone', 'last_sign_in_at', 'email_confirmed_at', 'confirmed_at', 'updated_at', 'profile', 'raw_user_meta_data'];
  ...
  const deleteAccount = async () => {
    ...
    await logout(); // Ensure logout is called after successful function invocation
    return data;
  };
